...
@Service
public class RescheduleLoanServiceImpl implements RescheduleLoanService {
    ...
    private final String DESCRIPTIONRESCHEDULE = "Se reagendó y se abrió una nueva cuenta con el siguiente identificador: ";

    @Override
    public Mono<ResponseAccountDTO> rescheduleLoan(String id, RescheduleDTO rescheduleDTO) {

        return mambuConfig.getClient().post()
                .uri("/loans/"+id+":reschedule")
                .body(Mono.just(rescheduleDTO), RescheduleDTO.class)
                ...
                .bodyToMono(ResponseAccountDTO.class).map( response -> {
                    response.setDescription(DESCRIPTIONRESCHEDULE + response.getId());
                    return response;
                })
                .onErrorResume(e -> {
