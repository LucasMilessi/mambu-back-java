...
import com.mambu.bancomambu.model.loan.ApproveDTO;
import com.mambu.bancomambu.model.loan.LoanDTO;
import com.mambu.bancomambu.service.active.ApproveLoanService;
...
@Service
public class ApproveLoanServiceImpl implements ApproveLoanService {
    ...
    private final String NOTE = "Prestamo aprobado";

    @Override
    public Mono<LoanDTO> approveLoan(String loanAccountId) {

        var approveDTO = ApproveDTO.builder()
                .action(APPROVE)
                .notes(NOTE)
                .build();

        return mambuConfig.getClient().post()
                .uri("/loans/"+loanAccountId+":changeState")
                .body(Mono.just(approveDTO), ApproveDTO.class)
